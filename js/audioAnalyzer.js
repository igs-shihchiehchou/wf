...
class AudioAnalyzer {
  ...
  constructor(audioContext) {
    ...
    this.cache = new Map();
    this.cachePrefix = 'audioAnalysis_';
    this.maxCacheSize = 10; // 最多緩存 10 個分析結果
  }

  /**
   * 計算音訊緩衝區的雜湊值
   *
   * 使用 SHA-256 算法計算音訊樣本數據的雜湊值，用於緩存鍵值。
   * 為了提高效能，僅採樣前 10 秒的音訊數據進行計算。
   *
   * @param {AudioBuffer} audioBuffer - 音訊緩衝區
   * @returns {Promise<string>} 雜湊值（十六進制字串）
   * @private
   */
  async computeAudioHash(audioBuffer) {
    try {
      // 獲取第一聲道的樣本數據
      const channelData = audioBuffer.getChannelData(0);

      // 為了性能考慮，僅採樣前 10 秒的數據（或全部數據，如果不足 10 秒）
      const sampleCount = Math.min(channelData.length, audioBuffer.sampleRate * 10);
      const samples = channelData.slice(0, sampleCount);

      // 將 Float32Array 轉換為 ArrayBuffer
      const buffer = samples.buffer;

      // 使用 SubtleCrypto API 計算 SHA-256 雜湊
      ...
      console.error('計算音訊雜湊時發生錯誤:', error);
      // 降級方案：使用基本資訊生成簡單的鍵值
      return `${audioBuffer.duration}_${audioBuffer.sampleRate}_${audioBuffer.length}`;
    }
  }

  /**
   * 從 localStorage 獲取緩存的分析結果
   *
   * @param {string} hash - 音訊雜湊值
   * @returns {Object|null} 緩存的分析結果，如果不存在則返回 null
   * @private
   */
  getCachedAnalysis(hash) {
    try {
      ...
      console.log('使用緩存的分析結果:', hash);
      return data.result;

    } catch (error) {
      console.error('讀取緩存時發生錯誤:', error);
      return null;
    }
  }

  /**
   * 將分析結果存入 localStorage
   *
   * @param {string} hash - 音訊雜湊值
  ...
  setCachedAnalysis(hash, result) {
    try {
      ...
      console.log('已緩存分析結果:', hash);

    } catch (error) {
      // localStorage 可能已滿或不可用
      console.warn('無法緩存分析結果:', error);
      ...
      if (error.name === 'QuotaExceededError') {
        ...
        // 再試一次
        try {
          localStorage.setItem(this.cachePrefix + hash, JSON.stringify(cacheData));
        } catch (e) {
          console.error('即使清理後仍無法緩存:', e);
        }
      }
    }
  }
  ...
  manageCacheSize() {
    try {
      ...
      if (cacheKeys.length > this.maxCacheSize) {
        ...
        // 刪除最舊的項目
        const toDelete = cacheEntries.length - this.maxCacheSize;
        for (let i = 0; i < toDelete; i++) {
          localStorage.removeItem(cacheEntries[i].key);
          console.log('已刪除舊緩存:', cacheEntries[i].key);
        }
      }
    } catch (error) {
      console.error('管理緩存大小時發生錯誤:', error);
    }
  }
  ...
  clearOldestCache() {
    try {
      ...
      if (cacheKeys.length > 0) {
        ...
        localStorage.removeItem(cacheKeys[0].key);
        console.log('已清除最舊的緩存:', cacheKeys[0].key);
      }
    } catch (error) {
      console.error('清除最舊緩存時發生錯誤:', error);
    }
  }
  ...
  clearAllCache() {
    try {
      ...
      console.log(`已清除 ${keysToRemove.length} 個緩存項目`);
   ...
   * @param {boolean} [options.useCache=true] - 是否使用緩存
   * @returns {Promise<AnalysisResult>} 分析結果物件，包含 { basic, frequency, pitch }
   */
  async analyze(audioBuffer, onProgress = () => {}, options = {}) {
    ...
    if (useCache) {
      onProgress(0, '檢查緩存...');
      const hash = await this.computeAudioHash(audioBuffer);
    ...
    try {
      ...
      if (useCache) {
        const hash = await this.computeAudioHash(audioBuffer);
        this.setCachedAnalysis(hash, result);
      }
...
window.audioAnalyzer = new AudioAnalyzer(window.audioProcessor.audioContext);
